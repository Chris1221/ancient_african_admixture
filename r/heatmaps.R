...
imf_heatmap = function(pop = "sgdp"){
  ...
  e_plot <- ggplot(e, aes(x = fct_reorder(factor(Var2),a$value), y = Var1, fill = value)) + 
    geom_tile() + 
    geom_text(aes(label=round(value, 3)), color="white") +
    xlab("African Group") +
    ylab("Eurasian Group") +
...
sgdp_imf_averages_table = function(){
  ...
  print( xtable(table, label = "average_sgdp_migration_table", caption = "Average plus or minus standard deviation integrated migration fraction (IMF) from Eurasian to African populations in the Simons Genome Diversity Panel between 30-70kya. Abbreviations: Integrated Migration Fraction (IMF), Stanard Deviation (SD), EUR (Eurasian), AFR (African)"), include.rownames = F)
}

papuan_linear_model = function(){
  tables = list()
  mats = make_imf_matrices(config_file = "~/repos/dirmig/analyses/hgdp_physically_phased.json", 
                           data_path = "~/repos/dirmig/data/hgdp_low_mig/",
                           pop = "hgdp")
  mat = mats[[1]] %>% reshape2::melt() %>% mutate(PapuanIndicator = grepl("Papuan", Var1, fixed = T))
  lm(value ~ PapuanIndicator + Var2, data = mat) %>% summary -> hgdp_pap
  
  hgdp_pap$coefficients %>% as.data.frame -> df
  df$`Pr(>|t|)` = scientific(df$`Pr(>|t|)`)
  hgdp_pap$coefficients = df
  tables[["hgdp"]] = xtable(hgdp_pap, caption = "Linear model predicting integrated migration fraction in the HGDP. A binary variable representing Papuan / not Papuan Eurasian donor and categorical variable representing African popualtion were used to predict the IMF.", label = "hgdp:papuan_imf")
    
  
  mats2 = make_imf_matrices()
  mat2 = mats2[[1]] %>% reshape2::melt() %>% mutate(PapuanIndicator = grepl("Papuan", Var1, fixed = T))
  lm(value ~ PapuanIndicator + Var2, data = mat2) %>% summary -> sgdp_pap 
  
  sgdp_pap$coefficients %>% as.data.frame -> df
  df$`Pr(>|t|)` = scientific(df$`Pr(>|t|)`)
  sgdp_pap$coefficients = df
  tables[["sgdp"]] = xtable(sgdp_pap, caption = "Linear model predicting integrated migration fraction in the SGDP. A binary variable representing Papuan / not Papuan Eurasian donor and categorical variable representing African popualtion were used to predict the IMF.", label = "hgdp:papuan_imf")
  #stargazer::stargazer(hgdp_pap, sgdp_pap, style = "all", single.row = T, column.labels = c("HGDP", "SGDP"))
