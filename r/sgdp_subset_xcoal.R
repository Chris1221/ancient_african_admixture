...
p <- ggplot(data = list_of_msmc %>% filter(source == "S_Han-1", sink != "S_Biaka-1"), aes(x = left_time_boundary, y = value, col = sink)) + 
  geom_vline(xintercept = seq(1e4,1e5,1e4), col = "grey", alpha = 0.3) +
  geom_vline(xintercept = seq(1e5,1e6,1e5), col = "grey", alpha = 0.3) +
  geom_step() + 
  scale_x_log10(labels = label_comma(scale = 0.001)) + 
...
q <- ggplot(data = list_of_msmc %>% filter(source == "S_Papuan-1", sink != "S_Biaka-1"), aes(x = left_time_boundary, y = value, col = sink)) + 
  geom_vline(xintercept = seq(1e4,1e5,1e4), col = "grey", alpha = 0.3) +
  geom_vline(xintercept = seq(1e5,1e6,1e5), col = "grey", alpha = 0.3) +
  geom_step() + 
  scale_x_log10(labels = label_comma(scale = 0.001)) + 
...
ggsave("~/repos/dirmig/plot/mig/papuan_xcoal.pdf", height = 4, width = 4, units = "in")
