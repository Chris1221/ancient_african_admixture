...
figure_1 = migration_plot(
  ...
  sims3 = figure_1_sim_plots()[[3]]) %>% 
    ggsave(file = "plot/new_mig_plot.pdf", width = 12, height = 6)
...
figure_2(
  ...
  ggsave(file = "plot/new_ne_figure.pdf", height = 8, width = 12)
...
make_all_histories_figure(
  ...
  ggsave(file="plot/individual_populations_averaged.pdf")
...
simple_imf_boxplot() %>% ggsave(file = "plot/mig/sgdp_averages.pdf", height = 5.61, width = 12.5, units = "in")
...
simulation_line_plot() %>% ggsave(file = "plot/sim_line_plot.pdf", h = 6, w = 10)
...
) %>% ggsave(file = "plot/new_dstats.pdf", height = 8, width = 12)
