  ...
  for(pr in prop){
    for(m in mid){
      ...
      p <- ggplot() + 
        ...
        scale_color_manual(values = c('blue', 'red', 'red', 'darkgrey', 'darkgrey'), labels = c("CEU", "YRI", "One-Population YRI", 'Truth', 'CEU Truth')) + 
...
pm + theme(legend.position = "bottom", legend.title = element_blank())


ggsave("~/repos/dirmig/plot/old_all_li_durbin.pdf", height = 10, width = 8, unit = "in")
