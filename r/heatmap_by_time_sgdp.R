...
mat = matrix(, nrow = n_sink, ncol = 20)

source = "S_French-1"
for (si in c(1:n_sink)){
  smcsmc_file = smcsmc(file = paste0(smc2_path, seed, ".", source, ".", sink[si], ".out"))
  mat[si, ] <- smcsmc_file@data %>% filter(Type == "Migr", Iter == max(Iter), Epoch < 20, From == 1) %>% select(Rate) %>% unlist %>% as.vector()
...
a <- reshape2::melt(t(mat)) %>% as.data.frame

a_plot <- ggplot(a, aes(x = Var1, y = fct_reorder(Var2, value), fill = value)) + 
  geom_tile() + 
  geom_text(aes(label = round(value / 1e-4, 3) ), size = 2, color="white") +
  xlab("African Group") +
  ylab("Eurasian Group") +
  scale_fill_gradient2(midpoint = 2e-4, mid = "red", low = "white")
theme_bw() + 
